...
pub struct NormalMapRegistryLoader {
    map: indexmap::IndexMap<String, AssetId<Image>, ahash::RandomState>,
}

impl NormalMapRegistryLoader {
    pub fn new() -> Self {
        Self {
    ...
    pub fn build_registry(
        ...
        for (label, id) in self.map.iter() {
            ...
            info!(
                "Normal map registry contains normal map '{label}' at {}",
                rect.min
            );
        }
        ...
        registry_map.extend(
            self.map
                ...
                .map(|(lbl, index)| (lbl, RegistryId::<NormalMapRegistry>::new(index as _))),
        );

        Ok(NormalMapRegistry {
            map: registry_map,
            atlas,
        })
    }
...
impl NormalMapRegistry {
    ...
    pub fn normal_map_position_buffer(&self) -> Vec<Vec2> {
...
pub struct NormalMapRegistryEntry<'a> {
    pub normal_map_pos: Vec2,

    // Placeholder in case we wanna store some other funny stuff in here
    _data: PhantomData<&'a ()>,
}

impl Registry for NormalMapRegistry {
    // GATs my beloved
    type Item<'a> = NormalMapRegistryEntry<'a>;
    ...
    fn get_by_id(&self, id: RegistryId<Self>) -> Self::Item<'_> {
        ...
        NormalMapRegistryEntry {
            normal_map_pos: self.atlas.textures[idx].min,
            _data: PhantomData,
        }
    }
...
mod tests {
    ...
    fn texture_registry_loading() {
        let loader = NormalMapRegistryLoader::new();
