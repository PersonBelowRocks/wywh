...
uvec3 extractVoxelPos(uint raw) {
    uint x = (raw & (0x1E000000 >> G_BS)) >> (UINT_BITS - (IDX_X+VXL_POS_CMPNT_SIZE) + G_BS);
    uint y = (raw & (0x01E00000 >> G_BS)) >> (UINT_BITS - (IDX_Y+VXL_POS_CMPNT_SIZE) + G_BS);
    uint z = (raw & (0x001E0000 >> G_BS)) >> (UINT_BITS - (IDX_Z+VXL_POS_CMPNT_SIZE) + G_BS);
...
uvec2 extractTexturePos(uint raw) {
    uint x = (raw & (0x1F800 >> G_BS)) >> (UINT_BITS - (IDX_TEXTURE_X+TEX_POS_CMPNT_SIZE) + G_BS);
    uint y = (raw & (0x007E0 >> G_BS)) >> (UINT_BITS - (IDX_TEXTURE_Y+TEX_POS_CMPNT_SIZE) + G_BS);
...
uint extractCornerId(uint raw) {
    return (raw & (0x18 >> G_BS)) >> (UINT_BITS - (IDX_CORNER+CORNER_SIZE) + G_BS);
}
...
void main() {
    ...
    // uint shouldBeOne = voxelData & 0x1;

    // assert(shouldBeOne == 0x1);
